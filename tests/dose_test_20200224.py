...
chip1.draw_dose_test_Nb(
    "dose_test",
    [PM.dose_pad, PM.dose_pad],
    PM.dose_sep,
    dose_mat,
    correction=PM.dose_cor,
    alum=False,
)
...
chip1.draw_alignement_marks(
    "align_dose_test", "80um", ["1.6mm", "1.8mm"]
)  # [0.6*width, 0.4*height]) #1.6 1.8mm
    ...
    for ii in range(2):  # 8
        ...
        for jj in range(0, 12):  # single junctions
            chip1.set_current_coor(
                [xpos, ypos + jj * PM.dose_sep + jj * PM.dose_pad + 0.5 * PM.dose_pad],
                [1, 0],
            )
            chip1.draw_dose_test_junction_corrected(
                "junction_temp",
                ["10um", "10um"],
                PM.dose_sep - PM.dose_cor + PM.overlap,
                junction_width_1[jj],
                PM.bridge_width,
                n_bridge=1,
                spacing_bridge=0,
    ...
    for ii in range(2, 5):
        ...
        for jj in range(0, 7):  # array junctions bridge spacing 1
            ...
            chip1.draw_dose_test_junction_corrected(
                ...
                ["10um", "10um"],
                PM.dose_sep - PM.dose_cor + PM.overlap,
                junction_array_width[indii],
                PM.bridge_width,
                n_bridge=3,
                spacing_bridge=bridge_spacing_tab[indjj],
                alternate_width=False,
                version=1,
                dose=True,
            )
    ...
    for ii in range(5, 7):
        for jj in range(0, 5):  # array junctions bridge spacing 2
            ...
            chip1.draw_dose_test_junction_corrected(
                ...
                ["10um", "10um"],
                PM.dose_sep - PM.dose_cor + PM.overlap,
                junction_width_2[indjj],
                PM.bridge_width,
                n_bridge=3,
                spacing_bridge=bridge_spacing_tab[1],
                alternate_width=False,
                version=1,
                dose=True,
            )
