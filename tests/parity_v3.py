...
if qubit:
    ...
    if litho:
        ...
        if ebeam:
            chip1.set_current_coor([PM.x_T, PM.y_T], [-1, 0])
            chip1.draw_dose_test_junction(
                "junction_trm",
                ["10um", PM.trm_junction_width],
                "50um",
                "0.147um",
                "400nm",
...
if parity:
    ...
    chip1.draw_selfparity(
        "parity",
        [PM.cutout_depth, PM.cutout_width],
        PM.cap_depth,
        PM.ind_gap,
        PM.jn_gap,
        PM.buffer,
        ...
        if ebeam:
            if 1:
                chip1.set_current_coor(
                    [
                        PM.x_I,
                        PM.y_I - 0.5 * PM.cutout_depth + 3 * PM.buffer + PM.cap_depth,
                    ],
                    [0, 1],
                )
                chip1.draw_cos2phi(
            ...
            if 1:
                chip1.set_current_coor(
                    [PM.x_I, PM.y_I - 0.5 * PM.cutout_depth + PM.buffer],
                    Vector([0, 1]).orth(),
                )
                chip1.draw_dose_test_junction(
                    "ind_parity",
...
if flux_lines:
    ...
    chip1.set_current_coor(
        [PM.x_I + PM.offset, PM.y_I + 0.5 * PM.cutout_depth - 1.75 * PM.approach],
        [0, 1],
    )
    chip1.draw_fluxline(
        "flux_top",
    ...
    chip1.set_current_coor(
        [PM.x_I + 0.5 * PM.cutout_width - PM.approach, PM.y_I - 0.55 * PM.offset],
        [1, 0],
    )
    chip1.draw_fluxline(
        "flux_bottom",
...
if drive_mem:
    ...
    chip1.draw_capa_inline(
        "capa_drive",
        PM.track,
        PM.gap,
        PM.len_capa_drive,
        0.5 * (PM.track - 3 * PM.track_mem),
        n_pad=3,
    )

    chip1.draw_cable(
        "chip1in_flux_topiOut",
        "chip1in_memiOut",
        "capa_drive_outPort2",
        is_bond=is_bond,
        fillet=PM.fillet,
...
if not (readout):
    ...
    chip1.double_port(
        "constrain_readout2",
        [PM.x_T - 0.3574 * PM.tune_ro, 0.5 * (PM.y_T + con1)],
        [0, -1],
        PM.track,
        PM.gap,
    )
    ...
    chip1.draw_cable(
        "readout",
        "trm_portOut2",
        "constrain_readout1_front",
        "constrain_readout1_back",
        "constrain_readout2_front",
        "constrain_readout2_back",
        "capa_readout_outPort1",
        is_bond=is_bond,
        fillet=PM.fillet,
    )
    chip1.draw_cable(
        "bef_capa",
        "capa_readout_outPort2",
        chip1.name + "in_readoutiOut",
        is_bond=is_bond,
        fillet=PM.fillet,
    )
#
