...
#                print('Warning: Found two overlapping points while creating the polyline, supressed one')
    ...
    def draw_trapeze(self, name, pos, z, size, height, angle=54.74*np.pi/180, base=True):
        ...
        # base is used to know if pos, size and z refer to the base or the top. 
...
#            self.new_layer('layer63')
    ...
    def draw_pits(self, rel_pos, length, width, layer):
        ...
        for ii in range(len(rel_pos)):
            if rel_pos[ii] in super().get_variable_names():
                        rel_pos[ii] = parse_entry(super(KeyElt,self).get_variable_value(rel_pos[ii]))
            else:
                continue
        ...
        self.layers[layer]['trackObjects'].append(pit_left)
        self.layers[layer]['trackObjects'].append(pit_right)
