...
def draw_JJ(self, name, iTrack, iGap, iTrackJ, iLength, iInduct='1nH', fillet=None):
    ...
    self.gapObjects.append(self.draw_rect_center(name+"_cutout", self.coor([0,0]), self.coor_vec([iLength, iTrack+2*iGap])))
...
#        junction = self.connect_elt(name+'_junction', name+'_in_jct', name+'_out_jct')
    junction_pad = self.connector.f('JJ', name+'_in_jct', name+'_out_jct', Jwidth+2*self.overdev, iInduct=Jinduc, fillet=None)
    ...
    pads = self.unite([right_pad, left_pad, junction_pads], name=name+'_pads')
    ...
    if nport==1:
#            self.ports[name+'_1'] = [self.coor(cutout_size.px()/2), self.ori, track, gap]
        self.port(name+'_1',cutout_size.px()/2, self.ori, track, gap)
    elif nport==2:
#            self.ports[name+'_1'] = [self.coor(cutout_size.px()/2), self.ori, track, gap]
#            self.ports[name+'_2'] = [self.coor(-cutout_size.px()/2), -self.ori, track, gap]
        self.port(name+'_1',cutout_size.px()/2, self.ori, track, gap)
        self.port(name+'_2',-cutout_size.px()/2, -self.ori, track, gap)
    elif nport==3:
#            self.ports[name+'_1a'] = [self.coor(cutout_size.px()/2+pad_size.py()/2), self.ori, track, gap]
#            self.ports[name+'_1b'] = [self.coor(cutout_size.px()/2-pad_size.py()/2), self.ori, track, gap]
#            self.ports[name+'_2'] = [self.coor(-cutout_size.px()/2), -self.ori, track, gap]
        self.port(name+'_1a',cutout_size.px()/2+pad_size.py()/2, self.ori, track, gap)
        self.port(name+'_1b',cutout_size.px()/2-pad_size.py()/2, self.ori, track, gap)
...
#            self.ports[name+'_1a'] = [self.coor(cutout_size.px()/2+pad_size.py()/2), self.ori, track, gap]
#            self.ports[name+'_1b'] = [self.coor(cutout_size.px()/2-pad_size.py()/2), self.ori, track, gap]
#            self.ports[name+'_2a'] = [self.coor(-cutout_size.px()/2+pad_size.py()/2), -self.ori, track, gap]
#            self.ports[name+'_2b'] = [self.coor(-cutout_size.px()/2-pad_size.py()/2), -self.ori, track, gap]
    elif nport==5:
        pass
#            self.ports[name+'_1'] = [self.coor(cutout_size.px()/2), self.ori, track+2*self.overdev, gap-2*self.overdev]
#            self.ports[name+'_2'] = [self.coor(-cutout_size.px()/2), -self.ori, track+2*self.overdev, gap-2*self.overdev]
#            self.ports[name+'_3a'] = [self.coor(Vector(pad_spacing/2+pad_size[0],-cutout_size[1]/2)), -self.ori.orth() ,track/2+2*self.overdev,gap/2-2*self.overdev]
#            if self.is_overdev:
#                sub_1 = self.draw_rect(name + '_sub_1', self.coor([cutout_size[0]/2, -track/2-gap+self.overdev]), self.coor_vec([-self.overdev, track+2*gap-2*self.overdev]))
#                sub_2 = self.draw_rect(name + '_sub_2', self.coor([-cutout_size[0]/2, -track/2-gap+self.overdev]), self.coor_vec([self.overdev, track+2*gap-2*self.overdev]))
#                sub_3a = self.draw_rect(name + '_sub_3a', self.coor([-track/2-gap+self.overdev+pad_spacing/2+pad_size[0],-cutout_size[1]/2]), self.coor_vec([track+2*gap-2*self.overdev, self.overdev]))
...
#        self.draw_rect_center(name+"check1", self.coor(cutout_size.px()/2)+self.ori*pad_size[0]/20, self.rot(*pad_size)/10)
#        self.draw_rect_center(name+"check2", self.coor(-cutout_size.px()/2)-self.ori*pad_size[0]/20, self.rot(*pad_size)/10)
#        self.draw_rect_center(name+"check3", self.coor(Vector(pad_spacing/2+pad_size[0],cutout_size[1]/2))+self.ori.orth()*pad_size[1]/20, self.rot(*pad_size)/10)
...
#        self.ports[name+'_out_jct'] = out_junction
#        junction = self.connect_elt(name+'_junction', name+'_in_jct', name+'_out_jct')
    junction_pads = self.network._connect_JJ('JJ', name+'_in_jct', name+'_out_jct', Jwidth+2*self.overdev, iInduct=Jinduc, fillet=None)
    ...
    if fillet is not None:
        ...
        if short_right!=0:
            ...
            right_quarter_up1 = self.draw_quarter_circle('right_quarter_up1', 'TRACK', fillet_right1)
            self.set_current_coor([cutout_size[0]/2-self.overdev, -(track_right/2+gap_right+short_right)-self.overdev], [0,-1])
            right_quarter_down1 = self.draw_quarter_circle('right_quarter_down1', 'TRACK', fillet_right1)
            ...
            right_quarter_up2 = self.draw_quarter_circle('right_quarter_up2', 'TRACK', fillet_right2)
            self.set_current_coor([cutout_size[0]/2-self.overdev, -(track_right/2+gap_right)+self.overdev], [0,-1])
            right_quarter_down2 = self.draw_quarter_circle('right_quarter_down2', 'TRACK' , fillet_right2)
            cutout = self.unite([cutout, right_quarter_up2[1], right_quarter_down2[1]], 'nom1')
        ...
        if short_left!=0:
            ...
            left_quarter_up1 = self.draw_quarter_circle('left_quarter_up1', 'TRACK', fillet_left1)
            self.set_current_coor( [-cutout_size[0]/2+self.overdev,-(track_left/2+gap_left+short_left)-self.overdev], [-1,0])
            left_quarter_down1 = self.draw_quarter_circle('left_quarter_down1', 'TRACK', fillet_left1)
            ...
            left_quarter_up2 = self.draw_quarter_circle('left_quarter_up2', 'TRACK', fillet_left2)
            self.set_current_coor([-cutout_size[0]/2+self.overdev,-(track_left/2+gap_left)+self.overdev], [-1,0])
            left_quarter_down2 = self.draw_quarter_circle('left_quarter_down2', 'TRACK', fillet_left2)
            cutout = self.unite([cutout, left_quarter_up2[1], left_quarter_down2[1]], 'nom2')
    ...
    if not self.is_litho:
        self.mesh_entity(mesh, 4*Jwidth)
...
def draw_capa(self, iTrack, iGap, pad_spacing, pad_size, half=False):
    ...
    raw_points = [(pad_spacing/2-self.overdev, pad_size[1]/2+self.overdev),
                  ...
                  (-pad_size[0]-2*self.overdev, 0)]
    points = self.append_points(raw_points)
    right_pad = self.draw(name+"_pad1", points)

    if not half:
        points = self.append_points(self.refy_points(raw_points))
        left_pad = self.draw(name+"_pad2", points)

    padlist = [right_pad]
    if not half:
        padlist.append(left_pad)
    pads = self.unite(padlist, name=name+'_pads')
    self.trackObjects.append(pads)

    jj = 2 if not half else 1
    pos_cutout = self.pos if not half else self.pos+self.ori*(pad_spacing/4+pad_size[0]/2+iGap/2)
    cutout = self.draw_rect_center(name+"_cutout", pos_cutout, self.coor_vec([pad_spacing*jj/2 + jj*pad_size[0]+jj*iGap-jj*self.overdev, pad_size[1] + 2*iGap-2*self.overdev]))
    
    if self.is_overdev:
        sub_1 = self.draw_rect(name + '_sub_1', self.coor([pad_spacing/2+pad_size[0]+iGap, -iTrack/2-iGap+self.overdev]), self.coor_vec([-self.overdev, iTrack+2*iGap-2*self.overdev]))
        if not half:
            sub_2 = self.draw_rect(name + '_sub_1', self.coor([-pad_spacing/2-pad_size[0]-iGap, -iTrack/2-iGap+self.overdev]), self.coor_vec([self.overdev, iTrack+2*iGap-2*self.overdev]))
    ...
    self.gapObjects.append(cutout)
    if self.is_mask:
        self.maskObjects.append(self.draw_rect_center(name+"_mask", self.coor([0,0]), self.coor_vec([pad_spacing + 2*pad_size[0]+4*iGap, pad_size[1] + 4*iGap])))
    if not self.is_litho:
        self.draw(name+"_mesh", points)
        self.modeler.assign_mesh_length(name+"_mesh",iTrack)
...
def draw_capa_inline(self, iTrack, iGap, capa_length, pad_spacing, n_pad=1, iTrack_capa=None, iGap_capa=None, premesh=True, tight=False): #iGap_capa is added gap
    ...
    drawn_pads = []
    if n_pad==1:
        drawn_pads.append(self.draw_rect(name+'_pad_left', self.coor([-capa_length/2,-iTrack/2-self.overdev]), self.coor_vec([capa_length/2-pad_spacing/2+self.overdev,iTrack+2*self.overdev])))
        drawn_pads.append(self.draw_rect(name+'_pad_right', self.coor([capa_length/2,-iTrack/2-self.overdev]), self.coor_vec([-(capa_length/2-pad_spacing/2)-self.overdev,iTrack+2*self.overdev])))
    else:
...
#                offset = iTrack
        ...
        for ii in range(int(n_pad/2)):
            drawn_pads.append(self.draw_rect(name+"_pad"+str(ii), self.coor([-capa_length/2+offset, curr_height-self.overdev]), self.coor_vec(pad_size+Vector([self.overdev, 2*self.overdev]))))
            drawn_pads.append(self.draw_rect(name+"_pad"+str(ii)+'b', self.coor([-capa_length/2+offset+pad_spacing-self.overdev, curr_height+pad_width+pad_spacing-self.overdev]), self.coor_vec(pad_size+Vector([self.overdev, 2*self.overdev]))))
            curr_height = curr_height+2*(pad_width+pad_spacing)
        if n_pad%2!=0:
            drawn_pads.append(self.draw_rect(name+"_pad"+str(ii+1), self.coor([-capa_length/2+offset, curr_height-self.overdev]), self.coor_vec(pad_size+Vector([self.overdev, 2*self.overdev]))))
        else:
            curr_height = curr_height-2*(pad_width+pad_spacing)
        if not usual:
#                drawn_pads.append(self.draw_rect(name+'_connect_left', self.coor([-capa_length/2-self.overdev, -iTrack_capa/2-self.overdev]), self.coor_vec([iTrack+2*self.overdev, curr_height+iTrack_capa/2+pad_width+2*self.overdev])))
            drawn_pads.append(self.draw_rect(name+'_connect_left', self.coor([-capa_length/2-self.overdev, -iTrack_capa/2-self.overdev]), self.coor_vec([offset+2*self.overdev, curr_height+iTrack_capa/2+pad_width+2*self.overdev])))
...
#                    drawn_pads.append(self.draw_rect(name+'_connect_right', self.coor([capa_length/2+self.overdev, -iTrack_capa/2+pad_width+pad_spacing-self.overdev]), self.coor_vec([-iTrack-2*self.overdev, curr_height+(iTrack_capa/2-2*pad_width-2*pad_spacing)+pad_width+2*self.overdev])))
                drawn_pads.append(self.draw_rect(name+'_connect_right', self.coor([capa_length/2+self.overdev, -iTrack_capa/2+pad_width+pad_spacing-self.overdev]), self.coor_vec([-offset-2*self.overdev, curr_height+(iTrack_capa/2-2*pad_width-2*pad_spacing)+pad_width+2*self.overdev])))
            else:
#                    drawn_pads.append(self.draw_rect(name+'_connect_right', self.coor([capa_length/2+self.overdev, -iTrack_capa/2+pad_width+pad_spacing-self.overdev]), self.coor_vec([-iTrack-2*self.overdev, curr_height+iTrack_capa/2+pad_width+2*self.overdev])))
                drawn_pads.append(self.draw_rect(name+'_connect_right', self.coor([capa_length/2+self.overdev, -iTrack_capa/2+pad_width+pad_spacing-self.overdev]), self.coor_vec([-offset-2*self.overdev, curr_height+iTrack_capa/2+pad_width+2*self.overdev])))
#                    
...
#                    drawn_pads.append(self.draw_rect(name+'_connect_left_b', self.coor([-capa_length/2-self.overdev, iTrack/2+self.overdev]), self.coor_vec([iTrack+2*self.overdev, -(iTrack/2+iTrack_capa/2)-2*self.overdev])))
                drawn_pads.append(self.draw_rect(name+'_connect_right_b', self.coor([capa_length/2+self.overdev, -iTrack/2-self.overdev]), self.coor_vec([-offset-2*self.overdev, (iTrack/2+iTrack_capa/2)+2*self.overdev])))
                drawn_pads.append(self.draw_rect(name+'_connect_left_b', self.coor([-capa_length/2-self.overdev, iTrack/2+self.overdev]), self.coor_vec([offset+2*self.overdev, -(iTrack/2+iTrack_capa/2)-2*self.overdev])))
...
#        else:
    ...
    if self.is_overdev:
        drawn_pads.append(self.draw_rect(name+'_added_right', self.coor([capa_length/2, -iTrack/2]), self.coor_vec([-self.overdev, iTrack])))
        drawn_pads.append(self.draw_rect(name+'_added_left', self.coor([-capa_length/2, -iTrack/2]), self.coor_vec([self.overdev, iTrack])))

    pads = self.unite(drawn_pads, name=name+'_pads')
    
    self.trackObjects.append(pads)
    
    self.gapObjects.append(self.draw_rect_center(name+"_gap", self.coor([0,0]), self.coor_vec([capa_length, iTrack + 2*iGap+2*iGap_capa-2*self.overdev])))
    if self.is_mask:
        self.maskObjects.append(self.draw_rect_center(name+"_mask", self.coor([0,0]), self.coor_vec([capa_length, iTrack + 2*iGap+2*iGap_capa +2*self.gap_mask])))
    
    if premesh:
        if not self.is_litho:
            self.draw_rect_center(name+"_mesh", self.coor([0,0]), self.coor_vec([capa_length, iTrack_capa+2*self.overdev]))
            self.modeler.assign_mesh_length(name+"_mesh",pad_spacing)
...
def draw_capa_interdigitated(self, iTrack, iGap, teeth_size,gap_size, N_period, fillet):
    ...
    connection = self.draw(name+"_capagap", points, closed=False)
    ...
    points=self.append_absolute_points(raw_points)
    capagap_starter = self.draw(name+'_width', points, closed=False)
    ...
    raw_points = [(-teeth_size[0]-iTrack-self.overdev, -N_teeth*teeth_size[1]-self.overdev),
                  ...
                  (teeth_size[0]+iTrack+self.overdev, -N_teeth*teeth_size[1]-self.overdev)]
    points = self.append_absolute_points(raw_points)
    pads = self.draw(name+"_pads", points)
    ...
    if not self.is_overdev:
        self.gapObjects.append(self.draw_rect_center(name+"_gap", self.coor([0,0]), self.coor_vec([2*teeth_size[0]+2*iTrack+2*iGap, 2*N_teeth*teeth_size[1]+2*iGap])))
    else:
    ...
    if self.is_mask:
        self.maskObjects.append(self.draw_rect_center(name+"_mask", self.coor([0,0]), self.coor_vec([2*teeth_size[0]+2*iTrack+4*iGap, 2*N_teeth*teeth_size[1]+4*iGap])))
...
#    print(self.ori)
    ...
    junction = self.connect_elt(name+'_junction_up', in_junction_up, out_junction_up)
    ...
    junction = self.connect_elt(name+'_junction_down', in_junction_down, out_junction_down)
    junction_pads_down = junction._connect_JJ(iTrackJ, iInduct=Lj_up, fillet=fillet)

    right_track = self.draw_rect_center(name+"_added_track1", self.coor([2*(squid_size[0]/2+adapt_dist),0]), self.coor_vec([squid_size[0]+2*adapt_dist, iTrack]))
    left_track = self.draw_rect_center(name+"_added_track2", self.coor([-2*(squid_size[0]/2+adapt_dist),0]), self.coor_vec([squid_size[0]+2*adapt_dist, iTrack]))

    squid = self.unite([right_track, left_track, track_a, track_b, track_c, track_d, junction_pads_down, junction_pads_up], name=name)
    ...
    self.gapObjects.append(self.draw_rect_center(name+"_added_gap", self.coor([0,0]), self.coor_vec([3*(squid_size[0]+2*adapt_dist), iTrack+2*iGap])))
    if self.is_mask:
        self.maskObjects.append(self.draw_rect_center(name+"_added_gap", self.coor([0,0]), self.coor_vec([3*(squid_size[0]+2*adapt_dist), iTrack+3*iGap])))
    ...
    if self.is_mask:
        ...
        self.maskObjects.append(self.draw(name+"_cutout_pump_a_mask", self.append_points(raw_points_adapt_pump_mask)))
        raw_points_adapt_pump_mask_b = self.refy_points(raw_points_adapt_pump_mask, offset = (squid_size[0]+2*adapt_dist)/2)
        self.maskObjects.append(self.draw(name+"_cutout_pump_b_mask", self.append_points(raw_points_adapt_pump_mask_b)))
    ...
    cutout_pump_a=self.draw(name+"_cutout_pump_a", points_adapt_pump_a)
    ...
    cutout_pump_b=self.draw(name+"_cutout_pump_b", points_adapt_pump_b)
    ...
    if doublePump:
        ...
        self.gapObjects.append(self.draw(name+"_cutout_pump_c", points_adapt_pump_c))
        ...
        self.gapObjects.append(self.draw(name+"_cutout_pump_d", points_adapt_pump_d))
    ...
    if doublePump:
        ...
        self.ports[name+'_pump1'] = portOutpump1
        portOutpump2 = [self.coor(Vector(pos_pump).refx()), -self.coor_vec(ori_pump), iTrackPump, iGapPump]
        self.ports[name+'_pump2'] = portOutpump2
    else:
        portOutpump1 = [self.coor(pos_pump), self.coor_vec(ori_pump), iTrackPump, iGapPump]
        self.ports[name+'_pump'] = portOutpump1
...
def draw_snails(self, iTrack, iGap, array_room, array_offset, iTrackPump, iGapPump, mode='litho', iTrackMinPump=None, iTrackSnail=None, fillet=None, N_snails=1, snail_dict={'loop_width':20e-6, 'loop_length':20e-6, 'length_big_junction':10e-6, 'length_small_junction':2e-6, 'bridge':1e-6, 'bridge_spacing':1e-6}, L_eq = '1nH'): #for now assume left and right tracks are the same width
    ...
    if mode=='litho':
        ...
        snail_array = self.connect_elt(name+'_array', out_array, in_array)
    ...
    if 0:
        in_array = [self.coor([array_room/2, array_offset]), -self.ori, iTrackSnail, 0]
        out_array = [self.coor([-array_room/2, array_offset]), self.ori, iTrackSnail, 0]      
        snail_array = self.connect_elt(name+'_array', in_array, out_array)
        snail_track = snail_array._connect_JJ(iTrack, iInduct=L_eq)
    
    if mode=='equivalent':
        connect_left = self.draw_rect(name+"_left", self.coor([array_room/2,array_offset-iTrackSnail/2]), self.coor_vec([-iTrackSnail, iTrackSnail]))
        connect_right = self.draw_rect(name+"_right", self.coor([-array_room/2,array_offset-iTrackSnail/2]), self.coor_vec([iTrackSnail, iTrackSnail]))
        array_eq = self.draw_rect_center(name+"_array_eq", self.coor([0,array_offset]), self.coor_vec([array_room-2*iTrackSnail, iTrackSnail]))
        ...
        self.draw(name+'_array_eq_line', points, closed=False)
...
#        junction_pads_down = junction._connect_JJ(iTrackJ, iInduct=Lj_up, fillet=fillet)
    ...
    right_track = self.draw_rect_center(name+"_added_track1", self.coor([2*(array_room/2+adapt_dist),0]), self.coor_vec([array_room+2*adapt_dist, iTrack+2*self.overdev]))
    left_track = self.draw_rect_center(name+"_added_track2", self.coor([-2*(array_room/2+adapt_dist),0]), self.coor_vec([array_room+2*adapt_dist, iTrack+2*self.overdev]))
    if mode=='equivalent':
        squid = self.unite([right_track, left_track, track_a, track_c, connect_left, connect_right], name=name+'_temp')
    else:
        squid = self.unite([right_track, left_track, track_a, track_c], name=name+'_temp')
    if fillet is not None:
        squid.fillet(iTrack/2,[0, 3, 8, 12])
        
    if 0:
        squid = self.unite([squid, snail_track], name=name)
    self.trackObjects.append(squid)
    ...
    gaps.append(self.draw_rect_center(name+"_added_gap", self.coor([0,0]), self.coor_vec([3*(array_room+2*adapt_dist), iTrack+2*iGap-2*self.overdev])))
    if self.is_mask:
        masks.append(self.draw_rect_center(name+"_added_gap", self.coor([0,0]), self.coor_vec([3*(array_room+2*adapt_dist), iTrack+2*iGap+2*self.gap_mask])))
    ...
    if self.is_mask:
        ...
        masks.append(self.draw(name+"_cutout_pump_a_mask", points_adapt_pump_a_mask))
    ...
    if self.is_mask:
        ...
        masks.append(self.draw(name+"_cutout_pump_b_mask", points_adapt_pump_b_mask))
    

    if doublePump:
        raw_points_adapt_pump_c = self.refx_points(raw_points_adapt_pump_a)
        points_adapt_pump_c = self.append_points(raw_points_adapt_pump_c)
        gaps.append(self.draw(name+"_cutout_pump_c", points_adapt_pump_c))
        if self.is_mask:
            ...
            masks.append(self.draw(name+"_cutout_pump_c_mask", points_adapt_pump_c_mask))

        raw_points_adapt_pump_d = self.refx_points(raw_points_adapt_pump_b)
        points_adapt_pump_d = self.append_points(raw_points_adapt_pump_d)
        gaps.append(self.draw(name+"_cutout_pump_d", points_adapt_pump_d))
        if self.is_mask:
            ...
            masks.append(self.draw(name+"_cutout_pump_d_mask", points_adapt_pump_d_mask))

    gaps = self.unite(gaps, name+'_cutout')
    self.gapObjects.append(gaps)
    if self.is_mask:
        masks = self.unite(masks, name+'_mask')
...
def draw_snails_Zaki(self, iTrack, iGap, array_room, array_offset, iTrackPump,
    ...
    if L_eq is not None:
        array_eq = self.draw_rect_center(name+"_array_eq", self.coor([0,array_offset]), self.coor_vec([array_room, iTrackSnail]))
        self.assign_lumped_RLC(array_eq, self.ori, (0, L_eq, 0))
...
#        junction_pads_down = junction._connect_JJ(iTrackJ, iInduct=Lj_up, fillet=fillet)
    ...
    right_track = self.draw_rect_center(name+"_added_track1", self.coor([2*(array_room/2+adapt_dist),0]), self.coor_vec([array_room+2*adapt_dist, iTrack]))
    left_track = self.draw_rect_center(name+"_added_track2", self.coor([-2*(array_room/2+adapt_dist),0]), self.coor_vec([array_room+2*adapt_dist, iTrack]))

    squid = self.unite([right_track, left_track, track_a, track_c], name=name)
    self.trackObjects.append(squid)
    ...
    self.gapObjects.append(self.draw_rect_center(name+"_added_gap", self.coor([0,0]), self.coor_vec([3*(array_room+2*adapt_dist), iTrack+2*iGap])))
    if self.is_mask:
        self.maskObjects.append(self.draw_rect_center(name+"_added_gap", self.coor([0,0]), self.coor_vec([3*(array_room+2*adapt_dist), iTrack+3*iGap])))
...
def draw_snails_sym(self, iTrack, iGap, array_room, iTrackPump, iGapPump, approach='20um', mode='litho', iTrackMinPump=None, iTrackSnail=None, fillet=None, N_snails=1, snail_dict={'loop_width':20e-6, 'loop_length':20e-6, 'length_big_junction':10e-6, 'length_small_junction':2e-6, 'bridge':1e-6, 'bridge_spacing':1e-6}, L_eq = '1nH'): #for now assume left and right tracks are the same width
    ...
    track_a = self.draw_rect(name+"_track_a", self.coor([array_room/2,-iTrackSnail/2]), self.coor_vec([adapt_dist, iTrackSnail]))
    track_c = self.draw_rect(name+"_track_c", self.coor([-array_room/2,-iTrackSnail/2]), self.coor_vec([-adapt_dist, iTrackSnail]))
    ...
    if mode=='equivalent':
        connect_left = self.draw_rect(name+"_left", self.coor([array_room/2,-iTrackSnail/2]), self.coor_vec([-iTrackSnail, iTrackSnail]))
        connect_right = self.draw_rect(name+"_right", self.coor([-array_room/2,-iTrackSnail/2]), self.coor_vec([iTrackSnail, iTrackSnail]))
        if not self.is_litho:
            array_eq = self.draw_rect_center(name+"_array_eq", self.coor([0,0]), self.coor_vec([array_room-2*iTrackSnail, iTrackSnail]))
            self.assign_lumped_RLC(array_eq, self.ori, (0, L_eq, 0))
...
#        junction_pads_down = junction._connect_JJ(iTrackJ, iInduct=Lj_up, fillet=fillet)
    ...
    right_track = self.draw_rect_center(name+"_added_track1", self.coor([2*(array_room/2+adapt_dist)+iTrackMinPump/2,0]), self.coor_vec([array_room+2*adapt_dist+iTrackMinPump, iTrack+2*self.overdev]))
    left_track = self.draw_rect_center(name+"_added_track2", self.coor([-2*(array_room/2+adapt_dist)-iTrackMinPump/2,0]), self.coor_vec([array_room+2*adapt_dist+iTrackMinPump, iTrack+2*self.overdev]))
    if mode=='equivalent':
        squid = self.unite([right_track, left_track, track_a, track_c, connect_left, connect_right], name=name+'_temp')
    else:
        squid = self.unite([right_track, left_track, track_a, track_c], name=name+'_temp')
    ...
    gap1 = self.draw(name+"_gap_1", points_gap)
    gap1.fillet(2*iTrackMinPump,[2,3])
    gaps.append(gap1)
    points_gap = self.append_points(self.refy_points(self.refx_points(raw_points_gap)))
    gap2 = self.draw(name+"_gap_2", points_gap)
    gap2.fillet(2*iTrackMinPump,[2,3])
    gaps.append(gap2)
    
#        gaps.append(self.draw_rect_center(name+"_added_gap", self.coor([0,0]), self.coor_vec([3*(array_room+2*adapt_dist)+2*iTrackMinPump, iTrack+2*iGap-2*self.overdev])))
    if self.is_mask:
        masks.append(self.draw_rect_center(name+"_added_gap", self.coor([0,0]), self.coor_vec([3*(array_room+2*adapt_dist), iTrack+2*iGap+2*self.gap_mask])))
    ...
    points_adapt_pump_c = self.append_points(raw_points_adapt_pump_c)
    cutout_pump_c = self.draw(name+"_cutout_pump_c", points_adapt_pump_c)
    ...
    points_adapt_pump_d = self.append_points(raw_points_adapt_pump_d)
    cutout_pump_d = self.draw(name+"_cutout_pump_d", points_adapt_pump_d)
...
#            cutout_pump_b.fillet(fillet/4,0)
    ...
    portOutpump1 = [self.coor(pos_pump), self.coor_vec(ori_pump), iTrackPump+2*self.overdev, iGapPump-2*self.overdev]
    self.ports[name+'_pump_1'] = portOutpump1
    ...
    self.ports[name+'_pump_2'] = portOutpump2
#        else:
#            portOutpump1 = [self.coor(pos_pump), self.coor_vec(ori_pump), iTrackPump+2*self.overdev, iGapPump-2*self.overdev]
#            self.ports[name+'_pump'] = portOutpump1
...
def draw_squid_protect(self, iTrack, iGap, squid_size, iTrackPump, iGapPump, iTrackSquid=None, iTrackJ=None, Lj_down='1nH', Lj_up=None,  typePump='down', doublePump=False, iSlope=1, iSlopePump=0.5, fillet=None): #for now assume left and right tracks are the same width
    ...
    track_out = self.draw(name+"_track_out", points_out)
    ...
    track_in = self.draw(name+"_track_in", points_in)
...
#    print(self.ori)
    ...
    right_track = self.draw_rect(name+"_added_track1", self.coor([squid_size[0]/2+iTrackSquid,-iTrack/2-self.overdev]), self.coor_vec([squid_size[0]+iTrackSquid, iTrack+2*self.overdev]))
    left_track = self.draw_rect(name+"_added_track2", self.coor([-squid_size[0]/2-iTrackSquid-squid_size[0]*2/3,-iTrack/2-self.overdev]), self.coor_vec([-squid_size[0]-iTrackSquid+squid_size[0]*2/3, iTrack+2*self.overdev]))

    squid = self.unite([right_track, left_track, track_out, junction_pads_down, junction_pads_up], name=name)
    self.trackObjects.append(squid)
    ...
    gaps.append(self.draw_rect_center(name+"_added_gap", self.coor([0,0]), self.coor_vec([3*squid_size[0]+4*iTrackSquid, iTrack+2*iGap-2*self.overdev])))
    if self.is_mask:
        masks.append(self.draw_rect_center(name+"_mask", self.coor([0,0]), self.coor_vec([3*squid_size[0]+4*iTrackSquid, iTrack+2*iGap+2*self.gap_mask])))
...
#            snail_track = snail_array._connect_JJ(iTrack, iInduct=L_eq)
    ...
    if mode=='equivalent':
        connect_left = self.draw_rect(name+"_left", self.coor([ind_length/2,-iTrack/2]), self.coor_vec([all_length/2-ind_length/2, iTrack]))
        connect_right = self.draw_rect(name+"_right", self.coor([-ind_length/2,-iTrack/2]), self.coor_vec([-(all_length/2-ind_length/2), iTrack]))
        if not self.is_litho:
            array_eq = self.draw_rect_center(name+"_eq", self.coor([0,0]), self.coor_vec([ind_length, iTrack]))
            self.assign_lumped_RLC(array_eq, self.ori, (0, L_eq, 0))
            points = self.append_points([(-ind_length/2,0),(ind_length,0)])
            self.draw(name+'_eq_line', points, closed=False)
 

    connect = self.unite([connect_left, connect_right])
    self.trackObjects.append(connect)
    
    gap = self.draw_rect(name+"_cutout", self.coor([-all_length/2,-iTrack/2-iGap]), self.coor_vec([all_length, iTrack+2*iGap]))
    self.gapObjects.append(gap)
    if self.is_mask:
        mask = self.draw_rect(name+"_mask", self.coor([-all_length/2,-iTrack/2-iGap-self.gap_mask]), self.coor_vec([all_length, iTrack+2*iGap+2*self.gap_mask]))
        self.maskObjects.append(mask)
...
#            snail_track = snail_array._connect_JJ(iTrack, iInduct=L_eq)
    ...
    island_center = self.draw(name+"_center", self.append_absolute_points(raw_points_center))
    island_right = self.draw(name+"_right",self.append_absolute_points( raw_points_right))
    island_left = self.draw(name+"_left", self.append_absolute_points(raw_points_left))
    island_center1 = self.draw(name+"_center1", self.append_absolute_points(raw_points_center1))
    island_center2 = self.draw(name+"_center2", self.append_absolute_points(raw_points_center2))
    island_left1 = self.draw(name+"_left1", self.append_absolute_points(raw_points_left1))
    island_left2 = self.draw(name+"_left2", self.append_absolute_points(raw_points_left2))
    island_right1 = self.draw(name+"_right1", self.append_absolute_points(raw_points_right1))
    island_right2 = self.draw(name+"_right2", self.append_absolute_points(raw_points_right2))
    connector_top = self.draw(name+"_conn_top", self.append_absolute_points(raw_points_connector_top))
    connector_left = self.draw(name+"_conn_left", self.append_absolute_points(raw_points_connector_left))
    connector_right = self.draw(name+"_conn_right", self.append_absolute_points(raw_points_connector_right))
    
    if not self.is_litho:
        array_eqS1 = self.draw_rect(name+"_LS1", self.coor([T/2+I, -T/2]), self.coor_vec([L, T]))
        array_eqS2 = self.draw_rect(name+"_LS2", self.coor([-T/2-I-L, -T/2]), self.coor_vec([L, T]))
        array_eqL = self.draw_rect(name+"_L", self.coor([-T/2, -T/2-IJ]), self.coor_vec([T, -L]))
        array_eqJ1 = self.draw_rect(name+"_LJ1", self.coor([2*I+L+T-T/2, -T/2-IJ]), self.coor_vec([T, -L]))
        array_eqJ2 = self.draw_rect(name+"_LJ2", self.coor([-2*I-L-T-T/2, -T/2-IJ]), self.coor_vec([T, -L]))
        ...
        self.draw(name+'_eq_lineS1', self.append_absolute_points(pointsS1), closed=False)
        pointsS2 = self.refy_points(pointsS1, absolute=True)
        self.draw(name+'_eq_lineS2', self.append_absolute_points(pointsS2), closed=False)
        pointsL = [(0,-T/2-IJ),(0,-T/2-IJ-L)]
        self.draw(name+'_eq_lineL', self.append_absolute_points(pointsL), closed=False)
        pointsJ1 = self.move_points(pointsL, [T+2*I+L, 0], absolute=True)
        self.draw(name+'_eq_lineJ1', self.append_absolute_points(pointsJ1), closed=False)
        pointsJ2 = self.move_points(pointsL, [-T-2*I-L, 0], absolute=True)
        self.draw(name+'_eq_lineJ2', self.append_absolute_points(pointsJ2), closed=False)
    ...
    gap = self.draw_rect(name+"_cutout", self.coor([-A/2,-T/2-C]), self.coor_vec([A, T+2*C]))
    self.gapObjects.append(gap)
#        if self.is_mask:
#            mask = self.draw_rect(name+"_mask", self.coor([-all_length/2,-iTrack/2-iGap-self.gap_mask]), self.coor_vec([all_length, iTrack+2*iGap+2*self.gap_mask]))
#            self.maskObjects.append(mask)
#            
    ...
    self.ports[name+'_T'] = portT
    self.ports[name+'_L'] = portL
    self.ports[name+'_R'] = portR
    self.ports[name+'_B'] = portB
...
def draw_T_join(self, iTrack, iGap):
    ...
    if not self.is_overdev or self.val(self.overdev<0):
        cutout = self.draw_rect_center(name+'_cutout', self.coor([0,self.overdev/2]), self.coor_vec([4*iGap+2*iTrack, 2*iGap+iTrack-self.overdev]))
        self.gapObjects.append(cutout)
    else:
        points = self.append_points([(-(iGap*2+iTrack),-iTrack/2-iGap+self.overdev),
                         ...
                         (0, self.overdev)])
        cutout = self.draw(name+'_cutout', points)
        self.gapObjects.append(cutout)
    
    if self.is_mask:
        mask = self.draw_rect(name+'_mask', self.coor([-iGap*2-iTrack,-iGap-iTrack/2]), self.coor_vec([4*iGap+2*iTrack, 2*iGap+iTrack+self.gap_mask]))
        self.maskObjects.append(mask)
    ...
    points = self.append_points([(-(iGap+iTrack/2)*2,-iTrack/2-self.overdev),
                                 ...
                                 (0, iGap-self.overdev)])
    track = self.draw(name+'_track', points)
...
def draw_fluxline(self, iTrack, iGap, length, track_flux, slope=0.5, sym='center', return_spacing=0, return_depth=0, opposite=False):
    ...
    if not sym == 'center':
        if return_spacing != 0:
            if not opposite:
                test1 = self.draw_rect(name+'_test1', self.coor([return_depth, length/2+offset_gap_up]), self.coor_vec([-(return_spacing+track_flux/2+return_depth), return_spacing+track_flux]))
            else:
                test1 = self.draw_rect(name+'_test1', self.coor([return_depth, -(length/2+offset_gap_up)]), self.coor_vec([-(return_spacing+track_flux/2+return_depth), -(return_spacing+track_flux)]))
    ...
    gap = self.draw(name+'_gap', points)
    
    
    track = self.draw(name+'_track_guide', points, closed=False)
    gapext = self.draw(name+'_gapext_guide', points, closed=False)
    fillet = eps
    ...
    track_starter = self.draw(name+'_track', points_starter, closed=False)
    gapext_starter = self.draw(name+'_gapext', points_starter, closed=False)
    ...
    if self.is_mask:
        mask = self.draw(name+'_mask', points)
        maskext = self.draw(name+'_mask_ext_guide', points, closed=False)
        points_starter_mask = self.append_absolute_points([(adapt_length, iGap+iTrack/2),
                                          (adapt_length, iGap+iTrack/2+self.gap_mask)])
        maskext_starter = self.draw(name+'_maskext', points_starter_mask, closed=False)
    ...
    track_adapt = self.draw(name+'_track_adapt', points)
    if is_fillet:
...
def draw_double_fluxline(self, iTrack, iGap, length, track_flux, slope=0.7, sym='center', return_spacing=0, return_depth=0, opposite=False):
    ...
    track_adapt_top = self.draw(name+'_track_adapt_top', points_top)
    track_adapt_middle = self.draw(name+'_track_adapt_middle',
                                   points_middle)
    track_adapt_bottom = self.draw(name+'_track_adapt_bottom',
                                   points_bottom)
...
#            track.fillet(fillet2, [17,20])
    ...
    self.ports[name+'_top'] = portOut_top
    self.ports[name+'_bottom'] = portOut_bottom
...
def draw_alignement_mark(self, iSize, iXdist, iYdist):
    ...
    raw_points = [(iXdist,iYdist),
                  (iSize/2,iSize/2),
                  (-iSize,0)]
    mark1=self.draw(name+"_mark_a", self.append_points(raw_points))
    
    raw_points = [(iXdist,iYdist),
                          (-iSize/2,-iSize/2),
                          (iSize,0)]
    mark2=self.draw(name+"_mark_b", self.append_points(raw_points))
...
def draw_alignement_mark_r(self, size, disp, suff=''):
    ...
    raw_points = [(*disp,),
                  ...
                  (0,-size)]        
    marks = []
    marks.append(self.draw(name+'_'+suff+"a", self.append_points(raw_points)))
    marks.append(self.draw(name+'_'+suff+"b", self.append_points(self.refy_points(raw_points, disp[0]))))
    ...
    if self.is_mask:
        ...
        marks_mask = []
        marks_mask.append(self.draw(name+suff+"a_mask", self.append_points(raw_points_mask)))
        marks_mask.append(self.draw(name+suff+"b_mask", self.append_points(self.refy_points(raw_points_mask, disp[0]))))
...
def draw_dose_test_Nb(self, pad_size, pad_spacing, array):
    ...
    for jj in range(array[1]):
        for ii in range(array[0]):
            pads.append(self.draw_rect(name+'_%d_%d'%(ii, jj), self.coor([pos_x, pos_y]), self.coor_vec(pad_size)))
    ...
    cutout = self.draw_rect(name+'_cutout', self.coor([0, 0]), self.coor_vec([pad_size[0]*2*array[0]+pad_spacing*3*array[0],pad_size[1]*array[1]+pad_spacing*(array[1]+1)]))
    if self.is_mask:
        mask = self.draw_rect(name+'_cutout', self.coor([-self.gap_mask, -self.gap_mask]), self.coor_vec([pad_size[0]*2*array[0]+pad_spacing*3*array[0]+2*self.gap_mask,pad_size[1]*array[1]+pad_spacing*(array[1]+1)+2*self.gap_mask]))
        self.maskObjects.append(mask)
...
def draw_dose_test(self, pad_size, pad_spacing, iTrack, bridge, N, bridge_spacing, length_big_junction, length_small_junction):
    ...
    self.draw_rect(name+'_left', self.coor([-pad_spacing/2, -pad_size[1]/2]), self.coor_vec([-pad_size[0],pad_size[1]]))
    self.draw_rect(name+'_right', self.coor([pad_spacing/2, pad_size[1]/2]), self.coor_vec([pad_size[0],-pad_size[1]]))
    ...
    in_array = portOut2
    out_array = portOut1
    snail_array = self.connect_elt(name+'_junction', in_array, out_array)
...
def draw_dose_test_junction(self, pad_size, pad_spacing, width, width_bridge, n_bridge=1, spacing_bridge=0, alternate_width=True):
    ...
    in_array = portOut2
    out_array = portOut1
    jcts = self.connect_elt(name+'_junction', in_array, out_array)
#    print(n_bridge)
    if alternate_width:
...
#        self.draw_rect(name+'_rect', self.coor([-size/2, -size/2]), self.coor_vec([size,size]))
    ...
    track = '42um'
    gap = '25um'
    name_s, name_e, name_n, name_w = name+'_s', name+'_e', name+'_n', name+'_w'
    port_nb = 0
    for ii, name in enumerate([name_s, name_e, name_n, name_w]):
        if ports_senw_bool[ii]:
            ...
            self.ports[name+'_port_'+str(port_nb)] = portOut
            
            end = self.key_elt(name+'_end_'+str(ii), self.coor([0, -size/2-tunnel_length+ports_senw[ii]]), self.coor_vec([0,-1]))
            end.draw_end_cable(track, gap, fillet=gap)
            cable = self.connect_elt(name+'_cable_'+str(ii), name+'_end_'+str(ii), name+'_port_'+str(port_nb))
...
def draw_dc_Nrect(self, layer_name, length, rel_pos, widths, border='10um'):
    ...
    for ii in range(mult):
        rect.append(self.draw_rect(layer_name+'_'+name+'_track_'+str(ii), \
                                    self.coor([-length/2, rel_pos[ii]-widths[ii]/2]), \
                                    self.coor_vec([length, widths[ii]])))
    if len(rect) > 1:
        rect = self.unite(rect, name=layer_name+'_'+name+'_track')
    else:
        rect = self.rename(rect[0], layer_name+'_'+name+'_track')
    ...
    cutout = self.draw_rect(layer_name+'_'+name+'_cutout', pos_cutout, vec_cutout)
    ...
    self.ports_dc[name+'_1'] = portOut1
    self.ports_dc[name+'_2'] = portOut2
...
def draw_dc_pad(self, layer_name, iTrack, iGap, xlength='250um', ylength='250um'):
    ...
    pad = self.unite([pad, padout], name=layer_name+'_'+name+'_track')
    pad_gap = self.draw_rect(layer_name+'_'+name+'_gap',\
...
def draw_dc_pads(self, layer_name, rel_pos, widths, gaps, xlength='250um', ylength='250um'):
    ...
    if len(pads) > 1:
        self.unite(pads, name=layer_name+'_'+name+'_track')
        self.unite(pad_gaps, name=layer_name+'_'+name+'_gap')
    else:
        self.rename(pads[0], layer_name+'_'+name+'_track')
        self.rename(pad_gaps[0], layer_name+'_'+name+'_gap')
...
def draw_alignment_marks(self, layer_name, isLayer63=True):
    ...
    for x0 in ['-42um', '42um']:
        ...
        for y0 in ['-42um', '42um']:
            ...
            mark = self.unite(mark, name=layer_name+'_'+name+'_alignement_mark_'+str(count))
            marks.append(mark)
            if isLayer63 is True:
                squares.append(self.draw_rect_center('layer63', self.coor([x0, y0]), self.coor_vec([square, square])))
                
    marks = self.unite(marks, name=layer_name+'_'+name+'_alignement_mark')
    self.layers[layer_name]['trackObjects'].append(marks)
    if isLayer63 is True:
        squares = self.unite(squares, name='layer63_'+name)
...
#    print(rel_pos)
    ...
    pit_right = self.draw_rect('pit_right', \
                                self.coor([-length, -dist2CNT-space]), \
                                self.coor_vec([2*length, -width]))
    pit_left = self.unite([pit_left, close_pit_left], name='pits_'+name+'_left')
    pit_right = self.unite([pit_right, close_pit_right], name='pits_'+name+'_right')
