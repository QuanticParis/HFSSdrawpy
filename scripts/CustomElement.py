...
class CustomElt():
    ...
    def draw_JJ(self, name, iTrack, iGap, iTrackJ, iLength, iInduct='1nH', fillet=None):
        ...
        portOut1 = Port(name+'_1',[iLength/2,0], [1,0], iTrack, iGap)
        portOut2 = Port(name+'_2',[-iLength/2,0], [-1,0], iTrack, iGap)
    ...
    def draw_IBM_tansmon(self,
        ...
        track_J=Jwidth*4.
        in_junction = Port(self.name+'_in_jct', [-pad_spacing/2+self.overdev, 0], [1,0], track_J+2*self.overdev, 0)
        out_junction = Port(self.name+'_out_jct', [pad_spacing/2-self.overdev, 0], [1,0], track_J+2*self.overdev, 0)
...
#            self.ports[self.name+'_1'] = [self.coor(cutout_size.px()/2), self.ori, track, gap]
            Port(self.name+'_1',cutout_size.px()/2, self.ori, track, gap)
        elif nport==2:
#            self.ports[self.name+'_1'] = [self.coor(cutout_size.px()/2), self.ori, track, gap]
#            self.ports[self.name+'_2'] = [self.coor(-cutout_size.px()/2), -self.ori, track, gap]
            Port(self.name+'_1',cutout_size.px()/2, self.ori, track, gap)
            Port(self.name+'_2',-cutout_size.px()/2, -self.ori, track, gap)
        elif nport==3:
...
#            self.ports[self.name+'_2'] = [self.coor(-cutout_size.px()/2), -self.ori, track, gap]
            Port(self.name+'_1a',cutout_size.px()/2+pad_size.py()/2, self.ori, track, gap)
            Port(self.name+'_1b',cutout_size.px()/2-pad_size.py()/2, self.ori, track, gap)
...
#        junction = self.connect_elt(self.name+'_junction', self.name+'_in_jct', self.name+'_out_jct')
        ...
        if fillet is not None:
            ...
            if short_left!=0:
                ...
                left_quarter_up = self.draw_quarter_circle('quarter_up_left2', [-cutout_size[0]/2+self.overdev,track_left/2+gap_left+short_left+self.overdev], [1,1], 'TRACK', fillet_left1)
                left_quarter_down = self.draw_quarter_circle('quarter_down2', [-cutout_size[0]/2+self.overdev,-(track_left/2+gap_left+short_left)-self.overdev], [1,-1], 'TRACK', fillet_left1)
                ...
                left_quarter_up = self.draw_quarter_circle('quarter_up_left1', [-cutout_size[0]/2+self.overdev,track_left/2+gap_left-self.overdev], [-1,1], 'TRACK', fillet_left2)
                left_quarter_down = self.draw_quarter_circle('quarter_down_left1', [-cutout_size[0]/2+self.overdev,-(track_left/2+gap_left)+self.overdev], [-1,-1], 'TRACK', fillet_left2)
        ...
        portOut1 = Port('portOut1', [cutout_size[0]/2,0], [1,0], track_right+2*self.overdev, gap_right-2*self.overdev)
#        self.ports[self.name+'_1'] = portOut1
        portOut2 = Port('portOut2', [-cutout_size[0]/2,0], [-1,0], track_left+2*self.overdev, gap_left-2*self.overdev)
