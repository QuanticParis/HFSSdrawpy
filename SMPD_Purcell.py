...
c.transmon.draw_IBM_tansmon([c.T_cutout_Width, c.T_cutout_Length],        #cutout_size_vec
                           ...
                           c.Lj,                                      #Jinduc                                                         #ori
                           nport=5,
                           fillet=c.T_fillet)                                 #Should be corrected for now doesn't make much sense
...
c.set_variable("capaW_length", '0.045mm')
...
c.squid.draw_squid(c.track, c.gap, [2*c.track, c.track], c.track/2, c.gap/2, iTrackSquid=c.track/4, iTrackJ=c.track/10,  Lj_down=c.L_SQUID, Lj_up=c.L_SQUID, typePump='down', doublePump=False)
