...
c.buffer_con.draw_connector(c.track, c.gap, bond_length, bond_slope, is_mask=is_mask)
c.waste_con.draw_connector(c.track, c.gap, bond_length, bond_slope, is_mask=is_mask)
c.qubit_con.draw_connector(c.track/2, c.gap/2, bond_length, bond_slope, is_mask=is_mask)
c.flux_con.draw_connector(c.track/2, c.gap/2, bond_length, bond_slope, is_mask=is_mask)
...
c.Control_cable.draw_cable(fillet=c.fillet_control, is_mask=is_mask), #is_meander=True, to_meander=[0, 1, 0, 1, 0, 1], meander_length='0.5mm') #for now does not support 2 meander in a row
...
c.capaPurW.draw_capa_interdigitated(c.track, c.gap, [c.capaPurW_teethlength, c.capaPurW_teethwidth],c.cappaPurW_gap,3,c.cappaPurW_fillet, is_mask=is_mask)                                                       #nport



######CONNECT WASTE CAPA to CONNECTOR
c.connect_elt('cable_Wcon_capa', 'waste_con', 'capaPurW_1')
c.cable_Wcon_capa.draw_cable(fillet=c.fillet, is_meander=False, is_mask=is_mask)
...
c.capaW.draw_capa(c.track, c.gap, c.capaW_spacing, [c.capaW_width, c.capaW_length], is_mask=is_mask)                                                       #nport
...
c.res_Waste.draw_cable(fillet=c.fillet, is_meander=True, to_meander=[0, -1, 0], meander_length=c.meander_length_W, ismesh=True,  is_mask=is_mask)

######CREATE PURCELL RESONATOR
c.set_variable('meander_length_PurW','1.29mm')
c.connect_elt('resPurcell_Waste', 'capaW_2', 'capaPurW_2')
c.resPurcell_Waste.draw_cable(fillet=c.fillet, is_meander=True, to_meander=[0, -1, 0], meander_length=c.meander_length_PurW, ismesh=True,  is_mask=is_mask)
...
c.capaB.draw_capa(c.track, c.gap, c.capaB_spacing, [c.capaB_width, c.capaB_length], is_mask=is_mask)                                                       #nport

######CONNECT BUFFER CAPA to CONNECTOR
c.connect_elt('cable_Bcon_capa', 'buffer_con', 'capaB_2')
c.cable_Bcon_capa.draw_cable(fillet=c.fillet, is_meander=False,  is_mask=is_mask)
...
c.squid.draw_squid(c.track, c.gap, [2*c.track, c.track], c.track/2, c.gap/2, iTrackSquid=c.track/4, iTrackJ=c.track/10,  Lj_down=c.L_SQUID, Lj_up=c.L_SQUID, typePump='down', doublePump=False, is_mask=is_mask, fillet=c.track/2)
...
c.res_Buffer_bef.draw_cable(fillet=c.fillet, is_meander=True, to_meander=[-1, 0], meander_length=c.meander_length_B, ismesh=True, is_mask=is_mask)
c.connect_elt('res_Buffer_aft', 'transmon_1', 'squid_1')
c.res_Buffer_aft.draw_cable(fillet=c.fillet, is_meander=True, to_meander=[-1, 0], meander_length=c.meander_length_B, ismesh=True, is_mask=is_mask)

######CREATE PUMP LINE
c.connect_elt('pump_line', 'flux_con', 'squid_pump')
c.pump_line.draw_cable(fillet=c.fillet, is_mask=is_mask)
